...
int main(void) {
  ...
  /****************************UART LAB PART 2****************************/
  xTaskCreate(uart__interrupt_read_task, "uart read task", 4096 / sizeof(void *), "data read", PRIORITY_LOW, NULL);
  xTaskCreate(uart__interrupt_write_task, "uart write task", 4096 / sizeof(void *),"data sent", PRIORITY_HIGH, NULL);
  vTaskStartScheduler();
  return 0;
}
...
void uart__interrupt_read_task(void *p) {
  while (1) {
    ...
    uart_lab__get_char_from_queue(&input_byte, 500);
    printf("Data read: %c\n\n", input_byte);
    vTaskDelay(500);
  }
}
