 ...
 * not allocate more memory or grow after initialization.
 *
 * @code
 *   char memory[64];
 ...
 *
 * This is useful such that we can use non-modifiable APIs with 'const
 * sl_string_s' such as sl_string__ends_with()
 *
...
/**
 * Similar to sl_string__c_str() but this API is to express the intent that the
 * returned cstring pointer is to be modfied. Care should be taken to only
 * modify contents based on sl_string__get_capacity()
 */
 ...
 * @note this will return true if string was constructed through
 * sl_string__initialize_non_modifiable()
 */
...
/**
 * Copy the string to the user provided pointer with max size of
 * 'copy_to_max_size' including NULL
 * @returns the number of bytes copied not including null terminator
 * @returns If the 'copy_to' is a NULL pointer, then this returns the number of
 * bytes that would have been copied
 */
...
/**
 * Append a string to sl_string_s string. If the string to be appended is larger
 * than the remaining capacity, the string to be appended will be truncated to
 * fit in the remaining space of sl_string_s string.
 * @returns the number of bytes copied not including null terminator
 */
...
/**
 * Search for `match_string` in `whole_string` up to either a NULL-terminating
 * character or `length_to_search` number of characters
 *
 * The behavior is similar to `strstr()`, but with an additional length limit of
 * `length_to_search` to search. See
 * https://www.freebsd.org/cgi/man.cgi?query=strnstr&sektion=3 for more detail
 * on expected behavior. NULL pointer parameters are also checked.
 *
 * @return Pointer to beginning of `match_string` match in `whole_string` if
