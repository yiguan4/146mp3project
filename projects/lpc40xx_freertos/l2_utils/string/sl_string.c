...
// lint -efile(901, *sl_string.c) args not initialized; va_list should never be
// initialized lint -efile(953, *sl_string.c) args could be const, va_list
// doesn't accept constants
...
static bool sl_string__private_can_be_modified(const sl_string_s input_string) {
  return (input_string.max_size > 0U);
}
...
static bool sl_string__private_ensure_enough_memory_for_insertion(
    const sl_string_s input_string, const char *insertion_string) {
 ...
 */
static char *sl_string__private_strtok_r(char *input_string,
...
sl_string_s sl_string__initialize_from(void *static_memory,
                                       size_t static_memory_size_max,
  ...
  if ((NULL != static_memory) && (NULL != copy_from)) {
    ...
    /* Assume it is okay if we fail to copy the string due to insufficient
     * memory This would be better than returning un-initialized string
     */
...
sl_string_s sl_string__initialize_non_modifiable(const void *existing_string) {
  /* 'const void*' parameter is used to express to the user that we will never
   * modify this input We cast it here, but never modify the string otherwise
   * because we set 'max_size' to zero
   */
...
bool sl_string__is_valid(const sl_string_s input_string) {
  return NULL != input_string.cstring;
}

size_t
sl_string__get_capacity_including_null_char(const sl_string_s input_string) {
  size_t capacity = 0;
...
double sl_string__to_float(const sl_string_s input_string) {
  return sl_string__is_valid(input_string) ? strtof(input_string.cstring, NULL)
                                           : 0.0f;
}
...
size_t sl_string__copy_to(const sl_string_s input_string, char *copy_to,
                          size_t copy_to_max_size) {
  size_t bytes_to_copy = 0;
  if (sl_string__is_valid(input_string)) {
    const size_t len = sl_string__get_length(input_string);
    const size_t bytes_to_copy_excluding_null =
        (copy_to_max_size > 0U) ? (copy_to_max_size - 1U) : 0U;
    bytes_to_copy = (len < bytes_to_copy_excluding_null)
                        ? len
                        : bytes_to_copy_excluding_null;
...
size_t sl_string__vprintf_append(sl_string_s input_string, const char *format,
                                 va_list args) {
  size_t chars_printed_val = 0;
  ...
  if (sl_string__is_valid(input_string) && (NULL != format) &&
    ...
    const size_t remaining_capacity =
        (sl_string__get_capacity_including_null_char(input_string) -
         existing_length);
    const int chars_printed = vsnprintf(input_string.cstring + existing_length,
...
int sl_string__tokenize(sl_string_s input_string, const char *delimiter_string,
                        int char_ptr_count, ...) {
  ...
  if (sl_string__is_valid(input_string) && (NULL != delimiter_string)) {
    if (sl_string__private_can_be_modified(input_string)) {
      ...
      char *active_token = sl_string__private_strtok_r(
          input_string.cstring, delimiter_string, &save_pointer);
    ...
    if (index_position <= sl_string__get_length(input_string)) {
      if (sl_string__private_ensure_enough_memory_for_insertion(input_string,
        ...
        char *const position_of_insertion =
            input_string.cstring + index_position;
...
size_t sl_string__append_truncate(sl_string_s input_string,
                                  const char *append) {
  ...
  if (sl_string__is_valid(input_string) && (NULL != append) &&
    ...
    const size_t length_of_original_string =
        sl_string__get_length(input_string);
    const size_t remaining_capacity =
        (sl_string__get_capacity(input_string) - length_of_original_string);
    const size_t number_of_chars_to_append =
        (length_of_insertion_string_without_null < remaining_capacity)
            ? length_of_insertion_string_without_null
            : remaining_capacity;
    // lint -esym(586, strncat) strncat is deprecated
    (void)strncat((input_string.cstring + length_of_original_string), append,
                  number_of_chars_to_append);
    appended_chars_count = number_of_chars_to_append;
  }
  return appended_chars_count;
}

bool sl_string__equals_to(const sl_string_s input_string,
...
int sl_string__last_index_of(const sl_string_s input_string,
                             const char *index_of) {
  const char *pointer_to_last = NULL;

  if (sl_string__is_valid(input_string) && (NULL != index_of)) {
    const char *pointer_to_first_in_substring =
        strstr(input_string.cstring, index_of);
  ...
  return ((NULL != pointer_to_last) ? (int)sl_utils__pointer_distance(
                                          pointer_to_last, input_string.cstring)
                                    : sl_string_error);
}

int sl_string__last_index_of_ignore_case(const sl_string_s input_string,
  ...
  if (sl_string__is_valid(input_string) && (NULL != index_of_case_ignored)) {
    ...
    if (NULL == pointer_to_first_char_in_substring) {
      pointer_to_first_char_in_substring =
          strchr(input_string.cstring, toupper((int)(*index_of_case_ignored)));
    }
    ...
    while (NULL != pointer_to_first_char_in_substring) {
      ...
      if (0 == strncasecmp(pointer_to_first_char_in_substring,
        ...
        index_of_string = (int)sl_utils__pointer_distance(
            pointer_to_first_char_in_substring, input_string.cstring);
        num_of_chars_to_move_substring_start_location =
            length_of_string_to_find;
      }

      const char *const pointer_to_last_char =
          pointer_to_first_char_in_substring;
...
int sl_string__first_index_of(const sl_string_s input_string,
                              const char *index_of) {
  ...
  return ((NULL != pointer_to_first)
              ? (int)sl_utils__pointer_distance(pointer_to_first,
                                                input_string.cstring)
              : sl_string_error);
}

int sl_string__first_index_of_ignore_case(const sl_string_s input_string,
                                          const char *index_of_case_ignored) {
  int index_of_string = sl_string_error;
  ...
  if (sl_string__is_valid(input_string) && (NULL != index_of_case_ignored)) {
    ...
    while (NULL != pointer_to_first_char_in_substring) {
      ...
      const char *const first_char_in_substring_storage_ptr =
          pointer_to_first_char_in_substring;
      pointer_to_first_char_in_substring =
...
bool sl_string__contains(const sl_string_s input_string,
...
bool sl_string__contains_ignore_case(const sl_string_s input_string,
                                     const char *substring_case_ignored) {
  int contains = sl_string_error;
  if (sl_string__is_valid(input_string) && (NULL != substring_case_ignored)) {
    contains = sl_string__first_index_of_ignore_case(input_string,
                                                     substring_case_ignored);
  }
  return (sl_string_error != contains);
}
...
int sl_string__count_of(const sl_string_s input_string, const char *count_of) {
  ...
  if (sl_string__is_valid(input_string) && (NULL != count_of)) {
    count = 0;
    const char *pointer_to_first_in_substring =
        strstr(input_string.cstring, count_of);
...
bool sl_string__begins_with(const sl_string_s input_string,
  ...
  if (sl_string__is_valid(input_string) && (NULL != begin_with)) {
    ...
    begins_with = sl_string__begins_with_limited_chars(input_string, begin_with,
                                                       substring_length);
  }
  return begins_with;
}
...
bool sl_string__begins_with_whole_word_ignore_case(
    const sl_string_s input_string, const char *begin_with_case_ignored,
    const char *delimiter_string) {
  bool result = false;
...
bool sl_string__ends_with(const sl_string_s input_string,
  ...
  if (sl_string__is_valid(input_string) && (NULL != ends_with)) {
    ...
    if (input_string_length >= substring_length) {
      if (0 ==
          strncmp(input_string.cstring + input_string_length - substring_length,
                  ends_with, substring_length)) {
        result = true;
      }
    }
  }
...
bool sl_string__ends_with_ignore_case(const sl_string_s input_string,
  ...
  if (sl_string__is_valid(input_string) && (NULL != ends_with_case_ignored)) {
    ...
    if (input_string_length >= substring_length) {
      if (0 == strncasecmp(input_string.cstring + input_string_length -
                               substring_length,
...
bool sl_string__erase(sl_string_s input_string, const char *erase) {
  ...
  if (sl_string__is_valid(input_string) && (NULL != erase) &&
      sl_string__private_can_be_modified(input_string)) {
    const int index_to_erase = sl_string__first_index_of(input_string, erase);
    if (sl_string_error != index_to_erase) {
      result = sl_string__erase_after(
          input_string, (size_t)(unsigned)index_to_erase, strlen(erase));
    }
  }
  return result;
}
...
bool sl_string__erase_last(sl_string_s input_string, size_t n_chars) {
  ...
  if (sl_string__is_valid(input_string) &&
      sl_string__private_can_be_modified(input_string)) {
    const size_t input_string_length = sl_string__get_length(input_string);
    const size_t start_of_erase_index =
...
bool sl_string__trim_start(sl_string_s input_string,
...
bool sl_string__replace_first(sl_string_s input_string, const char *replace,
                              const char *replace_with) {
  bool result = false;

  if (sl_string__is_valid(input_string) && (NULL != replace) &&
      (NULL != replace_with)) {
    if (sl_string__private_can_be_modified(input_string)) {
      const int start_of_replace_index =
          sl_string__first_index_of(input_string, replace);
      result = true;
...
bool sl_string__replace_last(sl_string_s input_string, const char *replace,
                             const char *replace_with) {
  bool result = false;
  ...
  if (sl_string__is_valid(input_string) && (NULL != replace) &&
    ...
    if (sl_string__private_can_be_modified(input_string)) {
      const int start_of_replace_index =
          sl_string__last_index_of(input_string, replace);
      result = true;
...
int sl_string__replace_all(sl_string_s input_string, const char *replace,
                           const char *replace_with) {
  ...
  if (sl_string__is_valid(input_string)) {
    ...
    if (sl_string__begins_with_ignore_case(input_string, "0x") &&
      ...
      for (size_t index = start_index; index < input_string_length; ++index) {
        if (isxdigit((int)(input_string.cstring[index])) >
            0) { // lint !e9029 ignoring mismatch type with binary ops
          is_hex = true;
        } else {
          is_hex = false;
          break;
...
bool sl_string__contains_only_decimal_number(const sl_string_s input_string) {
  ...
  if (sl_string__is_valid(input_string)) {
    ...
    for (size_t index = 0; index < input_string_length; ++index) {
      if (isdigit((int)(input_string.cstring[index])) >
          0) { // lint !e9029 ignoring mismatch type with binary operators
        is_decimal = true;
      } else {
        is_decimal = false;
        break;
...
bool sl_string__contains_only_number(const sl_string_s input_string) {
  ...
  if (sl_string__is_valid(input_string)) {
    ...
    is_number =
        (sl_string__contains_only_decimal_number(input_string) || is_number);
  }
  return is_number;
}

char *sl_string__strnstr(const sl_string_s whole_string,
                         const char *match_string, size_t length_to_search) {
  ...
  if (NULL != match_string) {
    ...
    if (!sl_string__is_valid(whole_string)) {
      ...
      for (size_t whole_string_offset = 0U;
           (whole_string_offset <= (length_to_search - match_string_length)) &&
           (!is_search_done);
           ++whole_string_offset) {
        char *const offset_string =
            (whole_string.cstring + whole_string_offset);

        if ('\0' == offset_string[0U]) {
          is_search_done = true;
        } else if (0 ==
                   strncmp(offset_string, match_string, match_string_length)) {
